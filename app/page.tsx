...
const BASE = 'https://www.elbehaus-luebeck.de/s/cc_images/';
const ACCENT = '#9a3b2e';
const ROTATED = [1338820];
const mk = (ids: number[], t: number, alt: string) =>
  ids.map((id, i) => ({ src: `${BASE}cache_${id}.JPG?t=${t}`, alt: `${alt} – Bild ${i + 1}`, rot: ROTATED.includes(id) }));
const eg = mk([1338566,1338567,1338568,1338569,1338570,1338571,1338572,1338573,1338574,1338575,1338576,1338577], 1471020686, 'Erdgeschoss');
const og = mk([1338669,1338670,1338671,1338672,1338673,1338674,1338675,1338676,1338677], 1471020909, '1. Obergeschoss');
const aussen = mk([1338817,1338818,1338819,1338820,1338821,1338822], 1471021144, 'Außenansicht, Treppenhaus und Innenhof');
const galleries = [
...
const plans = [
  { title: 'Kellergeschoss', img: `${BASE}cache_1337645.jpg?t=1471019034`, full: `${BASE}teaserbox_1337645.jpg?t=1471019034`, pdf: 'https://www.elbehaus-luebeck.de/app/download/2172433/KG+Vermietung+120816.pdf' },
  { title: 'Erdgeschoss', img: `${BASE}cache_1337689.jpg?t=1471019227`, full: `${BASE}teaserbox_1337689.jpg?t=1471019227`, pdf: 'https://www.elbehaus-luebeck.de/app/download/2172481/EG+Vermietung+120816.pdf' },
  { title: '1. Obergeschoss', img: `${BASE}cache_1337777.jpg?t=1471019333`, full: `${BASE}teaserbox_1337777.jpg?t=1471019333`, pdf: 'https://www.elbehaus-luebeck.de/app/download/2172525/1.OG+Vermietung+120816.pdf' },
];
      ...
      <section style={{ background: '#1d2733', color: '#e9e5dc', marginTop: 80 }}>
        <div style={{ maxWidth: 1180, margin: '0 auto', padding: '110px 48px', display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 80, alignItems: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <img src={`${BASE}cache_1338817.JPG?t=1471021144`} alt="Außenansicht" style={{ width: '100%', aspectRatio: '3/4', objectFit: 'cover', borderRadius: 2 }} />
            <img src={`${BASE}cache_1338566.JPG?t=1471020686`} alt="Wohnraum Erdgeschoss" style={{ width: '100%', aspectRatio: '3/4', objectFit: 'cover', borderRadius: 2, marginTop: 40 }} />
          </div>
        </div>
      </section>
